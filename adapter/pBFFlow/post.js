...
module.exports = {
    dependentNo: function (data) {
        console.log('--------------------dependentNo stage----------------' + JSON.stringify(data, null, 3))
        return new Promise(async (resolve, reject) => {
            try {
                ...
                data.tags.reportContent = { emergencyFund: '0', termInsurance: '0', healthInsurance: '0', homeloan: '0', investment: '0' }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (expense == 'invalid') {
                    ...
                    expensePt = await helper.expensePoint(data.tags.salary, data.tags.expense)
            ...
            try {
                ...
                if (emergencyAmt == 'invalid') {
                    ...
                    if (parseInt(data.tags.emergencyFund.current) >= parseInt(data.tags.emergencyFund.ideal)) {
                        ...
                        // data.tags.hindi.emergencyFund = msg.hindi.report.emergencyFund.ideal
                        data.tags.reportContent.emergencyFund = '1'

                    }
                    else {
                        data.tags.reportContent.emergencyFund = '0'
                        // data.tags.english.emergencyFund = msg.english.report.emergencyFund.notIdeal
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (data.data.toLowerCase() == 'skip') {
                    ...
                    idealInvestment = 0.2 * parseFloat(data.tags.salary)
                    data.tags.investment.ideal = idealInvestment
                    investPt = await helper.investPoint(data.tags.salary, data.tags.investment.current)
                    point = parseInt(data.tags.points) + parseInt(investPt)
                    data.tags.points = point
                    data.tags.reportContent.investment = '0'
                    ...
                    if (investAmt == 'invalid') {
                        ...
                        if (parseInt(data.tags.investment.current) < parseInt(data.tags.investment.ideal)) {
                            ...
                            data.tags.reportContent.investment = '1'
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (data.data.toLowerCase() == 'yes') {
                    ...
                    delete data.tags.healthInsuranceStage
                    if (parseInt(data.tags.salary) < 20000) {
                        data.stage = 'otherLoan'
                        console.log(data.tags)
                        return resolve(data)
                    }
                    data.stage = 'homeLoan'
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (emi == 'invalid') {
                    ...
                    data.tags.emi = emi
                    emiPt = await helper.emiPoint(data.tags.salary, data.tags.emi)
            ...
            try {
                ...
                if (data.data.toLowerCase() == 'skip') {
                    userData = {
                        ...
                        point: data.tags.points,
                        date: getDateInFormat(),
                        phone: 'null'
                    }
                    var addUserData = await dbManager.createPoints(userData)
                    delete data.tags.phoneStage
                    delete data.stage
